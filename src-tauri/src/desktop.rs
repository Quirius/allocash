...
#[tauri::command]
fn get_income_vs_expense(
    app: tauri::AppHandle,
    state: tauri::State<'_, BudgetState>,
    input: SpendingReportInput,
) -> Result<IncomeExpenseReport, String> {
    with_database(&app, &state, |database| {
        database.income_vs_expense(&input).map_err(ledger_error)
    })
}
#[tauri::command]
...
#[tauri::command]
fn get_outflow_over_time(
    app: tauri::AppHandle,
    state: tauri::State<'_, BudgetState>,
    input: OutflowOverTimeInput,
) -> Result<OutflowOverTimeReport, String> {
    with_database(&app, &state, |database| {
        database.outflow_over_time(&input).map_err(ledger_error)
    })
}
#[tauri::command]
